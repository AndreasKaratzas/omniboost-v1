...
from lib.estimator.utils import is_dist_avail_and_initialized
...
class MetricLogger(object):
    def __init__(self, log_dir: str, delimiter="\t"):
        self.meters = defaultdict(SmoothedValue)
        self.delimiter = delimiter
        self.export_filepath = Path(log_dir) / 'logger.log'
        self.log_message = []
    ...
    def _reserved_cuda_memory(self) -> int:
        ...
        for size in scales: 
            if mem_size > 1024:
                ...
                return int(round(mem_size)), size
        return int(round(mem_size)), size
    ...
    def log_every(self, iterable, epoch: int = None):
        ...
        for obj in iterable:
            ...
            if torch.cuda.is_available():
                log_dict['memory'], scale = self._reserved_cuda_memory()
            else:
                log_dict['memory'], scale = 0, 'B'
    ...
    def get_metrics(self):
        ...
        return self.stats.get('loss'), self.stats.get('accuracy')
        ...
        with open(self.export_filepath, "a") as f:
            for entry in self.log_message:
                f.write(
                    ...
                    f"{entry.get('accuracy'):15.3f}"
                    f"{entry.get('memory'):10d}\n"
                )

        self.log_message = []

class CustomAverageValueMeter:
    def __init__(self):
        self.reset()
    ...
    def add(self, value):
        ...
        self.std_dev = (self.sum_squared / self.count - self.mean ** 2) ** 0.5
