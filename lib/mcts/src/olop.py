...
from lib.mcts.utils.utils import kl_upper_bound
from lib.mcts.utils.factory import safe_deepcopy_env


class OpenLoopOptimisticPlanning(AbstractPlanner):
    ...
    def default_config(cls):
        cfg = super(OpenLoopOptimisticPlanning, cls).default_config()
        cfg.update(
            {
                "upper_bound":
                {
    ...
    def allocate_budget(self):
        ...
        self.config["episodes"], self.config["horizon"] = self.allocation(
            budget, self.config["gamma"])

    @staticmethod
    def allocation(budget, gamma):
        """Allocate the computational budget into `M`
        episodes of fixed horizon `L`.
        """
        for episodes in range(1, int(budget)):
            if episodes * OpenLoopOptimisticPlanning.horizon(episodes, gamma) > budget:
                episodes = max(episodes - 1, 1)
                horizon = OpenLoopOptimisticPlanning.horizon(episodes, gamma)
                break
        else:
            raise ValueError(
                "Could not split budget {} with gamma {}".format(budget, gamma))
        return episodes, horizon

    def run(self, state):
        """Run an OpenLoopOptimisticPlanning episode.
        Find the leaf with highest upper bound
        value, and sample the corresponding action 
        sequence.
        ...
        # Follow selection policy, expand tree if needed, collect 
        # rewards and update confidence bounds.
        node = self.root
        for h in range(self.config["horizon"]):
            ...
            if not node.children:  # Break ties at leaves
                ...
                action, _ = max(
                    [child for child in node.children.items()], key=lambda c: c[1].value_upper)
...
class OLOPNode(Node):
    ...
    def __init__(self, parent, planner):
        ...
        self.value_upper = (
            1 - gamma ** (self.planner.config["horizon"] + 1 - self.depth)) / (1 - gamma)
    ...
    def update(self, reward, done):
        if not 0 <= reward <= 1:
            raise ValueError(
                "This planner assumes that all rewards are normalized in [0, 1]")
    ...
    def compute_reward_ucb(self):
        ...
        if self.planner.config["upper_bound"]["type"] == "kullback-leibler":
            ...
            self.mu_ucb = kl_upper_bound(
                self.cumulative_reward, self.count, threshold)
        else:
            print("Unknown upper-bound type")
    ...
    def backup_to_root(self):
        ...
        if self.children:
            gamma = self.planner.config["gamma"]
            self.value_upper = self.mu_ucb + gamma * \
                np.amax([c.value_upper for c in self.children.values()])
        else:
